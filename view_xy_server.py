  ...
  <div id="wrap" class="max-w-6xl mx-auto px-4 sm:px-6 py-8">
    ...
    <div class="controls flex flex-wrap items-center gap-4 sm:gap-6 py-4 px-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200/80 dark:border-slate-600 shadow-sm">
      <div class="flex items-center gap-2">
        ...
        <input type="text" id="schoolSearch" placeholder="Search by name or acronym (e.g. HWC), comma for multiple, partial match highlighted" class="flex-1 min-w-[200px] rounded-lg border border-slate-300 dark:border-slate-500 px-3 py-2 text-sm text-slate-800 dark:text-slate-200 dark:bg-slate-700 placeholder-slate-400 dark:placeholder-slate-500 focus:border-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-400">
      </div>
      <div class="flex items-center gap-2">
  ...
  <script>
    ...
    const L = {
      ...
      zh: {
        ...
        schoolSearchPlaceholder: '按校名或首字母缩写（如 HWC）搜索，逗号分隔多个，部分匹配高亮',
    ...
    document.getElementById('schoolSearch').addEventListener('input', function() { refreshChart(); updateShareLinks(); });
    document.getElementById('schoolSearch').addEventListener('change', function() { refreshChart(); updateShareLinks(); });
